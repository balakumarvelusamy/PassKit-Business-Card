                            ...
                            {passes.map((pass) => (
                                <div key={pass.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem", background: "var(--background)", borderRadius: "12px", border: "1px solid var(--border)", flexWrap: "wrap", gap: "1rem" }}>
                                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                                        <span title={pass.key.split('/').pop()} style={{ fontWeight: 600, fontSize: "0.95rem", wordBreak: "break-all" }}>
                                            {(pass.key.split('/').pop() || "").length > 20
                                                ? (pass.key.split('/').pop() || "").substring(0, 20) + "..."
                                                : pass.key.split('/').pop()}
                                        </span>
                                        <span style={{ color: "var(--foreground-muted)", fontSize: "0.80rem" }}>
                                            {new Date(pass.lastModified).toLocaleString()}
                                        </span>
                                    </div>
                                    <div style={{ display: "flex", gap: "0.75rem" }}>
                                        <button onClick={() => handleDownload(pass.url, pass.key.split('/').pop() || 'pass.pkpass')} title="Download Pass" style={{ padding: "0.5rem", background: "rgba(107, 105, 105, 0.1)", color: "#ffffffff", border: "1px solid rgba(151, 149, 149, 0.2)", borderRadius: "8px", fontWeight: 600, fontSize: "0.85rem", cursor: deleting === pass.key ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
